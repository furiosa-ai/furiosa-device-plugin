...
.PHONY: image
image:
	docker build . -t registry.furiosa.ai/furiosa/furiosa-device-plugin:devel --progress=plain --platform=linux/amd64

.PHONY: image-no-cache
image-no-cache:
	docker build . --no-cache -t registry.furiosa.ai/furiosa/furiosa-device-plugin:devel --progress=plain --platform=linux/amd64

.PHONY: image-rel
image-rel:
	docker build . -t registry.furiosa.ai/furiosa/furiosa-device-plugin:latest --progress=plain --platform=linux/amd64

.PHONY: image-no-cache-rel
image-no-cache-rel:
	docker build . --no-cache -t registry.furiosa.ai/furiosa/furiosa-device-plugin:latest --progress=plain --platform=linux/amd64
...
e2e-inference-pod-image:
	docker build --build-arg FURIOSA_IAM_KEY=$(FURIOSA_IAM_KEY) --build-arg FURIOSA_IAM_PWD=$(FURIOSA_IAM_PWD) . -t registry.furiosa.ai/furiosa/furiosa-device-plugin/e2e/inference:latest --no-cache --progress=plain --platform=linux/amd64 -f ./e2e/inference_pod/Dockerfile
...
e2e-verification-image:
	docker build . -t registry.furiosa.ai/furiosa/furiosa-device-plugin/e2e/verification:latest --progress=plain --platform=linux/amd64 -f ./e2e/verification_pod/Dockerfile
